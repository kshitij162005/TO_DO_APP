...
function App() {
  ...
  const progressPercentage = Math.round((tasks.filter(t => t.completed).length / tasks.length) * 100);
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-6">
          ...
          <div className={`p-6 rounded-xl shadow-lg transition-colors duration-300 ${
            ...
            <div className="text-center">
              <h3 className="text-2xl font-bold">{time.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h3>
              <p className="text-4xl font-bold mt-2">{time.getDate()}</p>
              <p className="text-lg mt-2">{time.toLocaleDateString('en-US', { weekday: 'long' })}</p>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    {task.description && (
                      <p className={`text-sm mt-1 ${task.completed ? 'line-through text-gray-500' : 'text-gray-600 dark:text-gray-400'}`}>
                        {task.description}
                      </p>
                    )}
      ...
      {isAddingTask && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className={`bg-white p-6 rounded-xl shadow-lg w-full max-w-md ${darkMode ? 'bg-gray-800 text-white' : 'bg-gray-50 text-gray-900'}`}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-semibold">Add Task</h3>
              <button onClick={() => setIsAddingTask(false)} className="text-gray-500">
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            <div className="mb-4">
              <label htmlFor="task-category" className="block text-sm font-medium text-gray-700">Category</label>
              <select
                id="task-category"
            ...
            <div className="mb-4">
              <label htmlFor="task-time" className="block text-sm font-medium text-gray-700">Time Slot</label>
              <input
                type="text"
                id="task-time"
            ...
            <div className="mb-6">
              <label htmlFor="task-reminder" className="block text-sm font-medium text-gray-700">Reminder</label>
              <input
                type="time"
                id="task-reminder"
                value={newTask.reminder ? new Date(newTask.reminder).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
